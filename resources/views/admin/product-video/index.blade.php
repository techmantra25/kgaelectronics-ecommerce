...
<section>
    <div class="row">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $index + 1 }}</td>
								<td>{{$item->title}}</td>
								<td><img src="{{ asset($item->file_path) }}" width="50px" height="50px"></td>
                                <td class="file-holder">
                                    ...
                                    <div class="row__action">
                                        <a href="{{ route('admin.product-video.view', $item->id) }}">Edit</a>
                                        <a href="{{ route('admin.product-video.view', $item->id) }}">View</a>
                                        <a href="{{ route('admin.product-video.status', $item->id) }}">{{($item->status == 1) ? 'Active' : 'Inactive'}}</a>
                                        <a href="{{ route('admin.product-video.delete', $item->id) }}" onclick="return confirm('Are you sure?')" class="text-danger">Delete</a>
                                    </div>
                                </td>
        ...
        <div class="col-xl-4">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.product-video.store') }}" enctype="multipart/form-data">@csrf
                        ...
                        <div class="row">
                            <div class="col-md-12 card">
                                ...
                                <div class="card-body p-0">
                                    ...
                                    <input type="file" name="video" id="icon" accept="image/*" onchange="loadIcon(event)" class="d-none">
                                    <p class="small text-muted">Click here to browse video</p>
									<p class="small text-danger">Image Size: 1920px X 770px</p>
                                    <script>
                                        let loadIcon = function(event) {
                                            let iconOutput = document.getElementById('iconOutput');
                                            iconOutput.src = URL.createObjectURL(event.target.files[0]);
